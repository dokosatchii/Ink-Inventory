...
$hpprinter=$_POST['posthptype'];
$hpcolor=$_POST['posthpcolor'];
$amount=$_POST['posthpamount'];

//connecting to MySQL
mysql_connect($server,$user_name,$password) or die(mysql_error());
mysql_select_db($database);
$query = "SELECT * FROM hp WHERE printer='$hpprinter' OR alt_printer='$hpprinter' AND color='$hpcolor'";
...
if($row['quantity']==0)
{
	...
	$result=mysql_query("UPDATE hp SET quantity = quantity-$amount WHERE printer='$hpprinter' OR alt_printer='$hpprinter' AND color='$hpcolor'") or die(mysql_error());

	//creates a text log for todd to view later
	$data=$ticket." ".$department." ".$hpprinter." ".$hpcolor." - ".$amount." ".$date."\r\n";
	$log="log.txt";
